<?php
/**
 * DoWaba AI manifest endpoint
 * URL: /index.php?fc=module&module=dowaba_ai&controller=manifest
 * Class adı: ModuleNameModuleControllerNameModuleFrontController
 *            (PrestaShop convention: filename = "manifest", class = "Dowaba_AiManifestModuleFrontController")
 */
...
class Dowaba_AiManifestModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $manifest = [
            ...
            'functions' => [
                $this->fn('opc_product_search', 'Ürün ara', 'Ad/SKU/kategoriye göre ürün listele.', 'read',
                ...
                $this->fn('opc_product_detail', 'Ürün detayı', 'Tek ürün tam bilgi.', 'read',
                    ['type' => 'object', 'properties' => ['product_id' => ['type' => 'integer']], 'required' => ['product_id']],
                    'GET', 'product', ['id' => '{{arg.product_id}}'], null, 5000
                ),
                $this->fn('opc_product_compare', 'Ürün karşılaştır', '2-3 ürün yan yana.', 'read',
                    ['type' => 'object', 'properties' => [
                        'product_ids' => ['type' => 'array', 'items' => ['type' => 'integer'], 'minItems' => 2, 'maxItems' => 3],
                    ], 'required' => ['product_ids']],
                    'GET', 'compare', ['ids' => '{{arg.product_ids}}'], null, 8000
                ),
                $this->fn('opc_stock_check', 'Stok kontrol', 'Ürünün stok adedi.', 'read',
                    ['type' => 'object', 'properties' => [
                ...
                $this->fn('opc_category_list', 'Kategori liste', 'Kategori ağacı.', 'read',
                    ['type' => 'object', 'properties' => ['parent_id' => ['type' => 'integer', 'default' => 0]]],
                    'GET', 'categories', ['parent_id' => '{{arg.parent_id}}'], null, 5000
                ),
                $this->fn('opc_order_status', 'Sipariş durumu', 'Email match ile.', 'read',
                    ['type' => 'object', 'properties' => [
                        'order_id' => ['type' => 'integer'],
                        'email' => ['type' => 'string'],
                    ], 'required' => ['order_id', 'email']],
                    'GET', 'order', ['id' => '{{arg.order_id}}', 'email' => '{{arg.email}}'], null, 5000
                ),
                $this->fn('opc_customer_lookup', 'Müşteri sorgu', 'Telefon/email ile.', 'read',
                    ['type' => 'object', 'properties' => [
                        'phone' => ['type' => 'string'],
                        'email' => ['type' => 'string'],
                    ]],
                    'GET', 'customer_lookup', ['phone' => '{{arg.phone}}', 'email' => '{{arg.email}}'], null, 5000
                ),
                $this->fn('opc_cart_recover', 'Sepet hatırlat', 'Re-engagement link.', 'read',
                    ['type' => 'object', 'properties' => [
                        'email' => ['type' => 'string'],
                        'customer_id' => ['type' => 'integer'],
                    ]],
                    'POST', 'cart_recover', null,
                    ['email' => '{{arg.email}}', 'customer_id' => '{{arg.customer_id}}'], 5000
                ),
                $this->fn('opc_order_preview', 'Sipariş önizle', 'KRİTİK: müşteri onayı öncesi özet.', 'write',
                    ['type' => 'object', 'properties' => [
                        'items' => ['type' => 'array'],
                        'customer' => ['type' => 'object'],
                    ], 'required' => ['items', 'customer']],
                    'POST', 'order_preview', null,
                    ['items' => '{{arg.items}}', 'customer' => '{{arg.customer}}'], 8000
                ),
                $this->fn('opc_order_confirm', 'Sipariş onayla', 'KRİTİK: müşteri "evet" sonrası.', 'write',
                    ['type' => 'object', 'properties' => [
                        'preview_id' => ['type' => 'string'],
                        'confirmed' => ['type' => 'boolean', 'default' => true],
                    ], 'required' => ['preview_id']],
                    'POST', 'order_confirm', null,
    ...
    private function fn(string $slug, string $name, string $desc, string $scope, array $parameters, string $method, string $action, ?array $query_extra, ?array $body_template, int $timeout, ?array $response = null): array
    {
        ...
        if ($query_extra) foreach ($query_extra as $k => $v) $query[$k] = $v;
        ...
        if ($body_template !== null) $config['body_template'] = $body_template;
