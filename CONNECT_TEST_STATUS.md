...
| **Magento** | ✅ (manifest pre-filled) | ⚙️ kod-doğrulandı (env ağır) | 2026-06-01 | — (kaynak, release kaldı) |
| Shopify | — (OAuth modeli) | yok — zaten tek-tık "authorize" | — | — |
| İkas | — (OAuth modeli) | yok — zaten tek-tık "authorize" | — | — |

**Özet:** Bundle-Import eklentilerinin (Presta/Woo/OC/Magento) hepsine deep-link butonu eklendi. **3'ü canlı test edildi** (Presta uçtan uca import, Woo + OC4 render+href). Magento aynı desen, kod-doğrulandı (env'i ağır olduğu için canlı render bekliyor). Shopify/İkas OAuth olduğu için zaten tek-tık.
...
| Magento | kurulu DEĞİL (OpenSearch + ~30-45dk install) | — |
...
- **WooCommerce / OpenCart / Magento**: version bump + zip + GitHub release. (Değişiklikler test edildi/doğrulandı, düşük riskli.)
- **Magento**: istenirse Docker'da tam kurulup canlı render testi.
